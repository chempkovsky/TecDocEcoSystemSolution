// CarShop.Controllers.ReturnReportController
...
namespace CarShop.Controllers
{
    ...
    public class ReturnReportController : Controller
    {
        ...
        protected SelectList SelectListHelper(int showIs)
        {
            ...
            {
                Value = "1",
                Text = Resources.ReportTypeReturned,
                Selected = (showIs == 1)
            },
            new SelectListItem
            {
                Value = "2",
                Text = Resources.ReportTypeNotReturned,
                Selected = (showIs == 2)
            },
            new SelectListItem
            {
            ...
            {
                Value = "4",
                Text = Resources.ReturnBasketReportIncomeTDES_INDEX,
                Selected = (showIs == 4)
            }
            }, "Value", "Text", showIs);
        }
        ...
        public async Task<ActionResult> Index(Guid? searchEntGuid = default(Guid?), Guid? searchEntBranchGuid = default(Guid?), Guid? SpellGuid = default(Guid?), int? reportType = default(int?))
        {
            ...
            else
            {
                ...
                {
                    case 1:
                        aResult = await (from e in dbSales.ReturnBasketTDES
                                         ...
                                         select new SaleBasketReportTDES
                                         {
                                             ...
                                             PaymentSum = grpSpell.Sum((ReturnBasketTDES ei) => ei.PaymentSum),
                                             ArtAmount = grpSpell.Sum((ReturnBasketTDES ei) => ei.ArtAmount)
                                         }).ToListAsync();
                        break;
                    case 2:
                        aResult = await (from e in dbSales.ReturnBasketTDES
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        aResult = await (from e in dbSales.ReturnBasketArticleTDES
                                         ...
                                         select new SaleBasketReportTDES
                                         {
                                             ...
                                             WorkPlaceGuid = grpSpell.Key.WorkPlaceGuid,
                                             EntUserNic = grpSpell.Key.EntUserNic,
                                             PaymentSum = grpSpell.Sum((ReturnBasketArticleTDES ei) => ei.SalePrice * (double)(ei.ArtAmount - ei.ReverseAmount)),
                                             ArtAmount = grpSpell.Sum((ReturnBasketArticleTDES ei) => ei.ArtAmount - ei.ReverseAmount)
                                         }).ToListAsync();
                        break;
                }
            }
